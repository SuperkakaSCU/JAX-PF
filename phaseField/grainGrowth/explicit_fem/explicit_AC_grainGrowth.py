"""
Explicit finite element solver for Allen Cahn
...
class AllenCahnMass(Problem):
    ...
    def get_universal_kernel(self):
        def universal_kernel(cell_sol_flat, x, cell_shape_grads, cell_JxW, cell_v_grads_JxW):
            ...
            ###################### (num_nodes_p, vec_p)
            dt = self.params['dt']
            tmp3_0 = n0 / dt # (num_quads,)
            tmp3_1 = n1 / dt # (num_quads,)
            tmp3_2 = n2 / dt # (num_quads,)
            tmp3_3 = n3 / dt # (num_quads,)
            tmp3_4 = n4 / dt # (num_quads,)
            tmp3_5 = n5 / dt # (num_quads,)
            ...
            weak_form = [val3_0, val3_1, val3_2, val3_3, val3_4, val3_5] # [(num_nodes_n0, vec_n0), (num_nodes_n1, vec_n1), ...]
...
class AllenCahnForce(Problem):
    ...
    def get_universal_kernel(self):
        ...
        def universal_kernel(cell_sol_flat, x, cell_shape_grads, cell_JxW, cell_v_grads_JxW):
            ...
            n0_grads = np.sum(cell_sol_n0[None, :, :, None] * cell_shape_grads_n0[:, :, None, :], axis=1) # (num_quads, vec_p, dim) -> (4, 1, 2)
            n1_grads = np.sum(cell_sol_n1[None, :, :, None] * cell_shape_grads_n1[:, :, None, :], axis=1) # (num_quads, vec_p, dim) -> (4, 1, 2)
            n2_grads = np.sum(cell_sol_n2[None, :, :, None] * cell_shape_grads_n2[:, :, None, :], axis=1) # (num_quads, vec_p, dim) -> (4, 1, 2)
            n3_grads = np.sum(cell_sol_n3[None, :, :, None] * cell_shape_grads_n3[:, :, None, :], axis=1) # (num_quads, vec_p, dim) -> (4, 1, 2)
            n4_grads = np.sum(cell_sol_n4[None, :, :, None] * cell_shape_grads_n4[:, :, None, :], axis=1) # (num_quads, vec_p, dim) -> (4, 1, 2)
            n5_grads = np.sum(cell_sol_n5[None, :, :, None] * cell_shape_grads_n5[:, :, None, :], axis=1) # (num_quads, vec_p, dim) -> (4, 1, 2)
            ...
            ##################### (num_nodes_p, vec_p)
            # Handles the term `f_grad*q*dx` [Explicit]
            ...
            weak_form = [val1_0+val2_0, val1_1+val2_1, val1_2+val2_2, val1_3+val2_3, val1_4+val2_4, val1_5+val2_5] # [(num_nodes_p, vec_p), (num_nodes_T, vec_T)]
...
def simulation():
    ...
    problem_mass = AllenCahnMass([mesh, mesh, mesh, mesh, mesh, mesh], vec=[1, 1, 1, 1, 1, 1], dim=2, \
                                    ele_type=[ele_type, ele_type, ele_type, ele_type, ele_type, ele_type], \
                                    additional_info=[params])

    problem_force = AllenCahnForce([mesh, mesh, mesh, mesh, mesh, mesh], vec=[1, 1, 1, 1, 1, 1], dim=2, \
                                    ele_type=[ele_type, ele_type, ele_type, ele_type, ele_type, ele_type], \
                                    additional_info=[params])


    ### Hu: Positions of fe_n0
    points = problem_mass.fe_n0.points
    

    ## Hu: Definition of initial condition
    def set_ICs(sol_IC, p, domain_size, index, center, rad):
        p = p.reshape(nx+1, ny+1, problem_mass.fes[0].dim)
    ...
    # (num_vars, nx, ny)
    sol_IC = np.array([np.zeros((nx+1, ny+1))] * 6)

    # 15 index for definition of 6 order params (0~5)
    # (num_nucli, )
    ...
    sol_IC_nucl = sol_IC_nucl.reshape(problem_mass.num_vars, -1)

    
    sol_list = [sol_IC_nucl[0].reshape(problem_mass.fe_n0.num_total_nodes, problem_mass.fe_n0.vec), 
                sol_IC_nucl[1].reshape(problem_mass.fe_n1.num_total_nodes, problem_mass.fe_n1.vec),
                sol_IC_nucl[2].reshape(problem_mass.fe_n2.num_total_nodes, problem_mass.fe_n2.vec),
                sol_IC_nucl[3].reshape(problem_mass.fe_n3.num_total_nodes, problem_mass.fe_n3.vec),
                sol_IC_nucl[4].reshape(problem_mass.fe_n4.num_total_nodes, problem_mass.fe_n4.vec),
                sol_IC_nucl[5].reshape(problem_mass.fe_n5.num_total_nodes, problem_mass.fe_n5.vec)]
...
if __name__ == '__main__':
    ...
    print("This is explicit solver for grain growth:", end_time - start_time)
