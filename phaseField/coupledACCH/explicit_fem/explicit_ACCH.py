"""
Explicit finite element solver for coupling Allen Cahn and Cahn Hilliard
Note: we don't have the gradient term for the composition, i.e., gradient c terms
"""
import jax
...
class CoupledACCHMass(Problem):
    ...
    def get_universal_kernel(self):
        def universal_kernel(cell_sol_flat, x, cell_shape_grads, cell_JxW, cell_v_grads_JxW):
            ...
            ######################
            ## This is phase field variable weak form for c and n
            ######################
            dt = self.params['dt']

            ## Hu: p & p_old: (num_quads,)
            tmp3_c = c / dt # (num_quads,)
            tmp3_n = n / dt # (num_quads,)
            ...
            weak_form = [val3_c, val3_n] # [(num_nodes_n, vec_n), (num_nodes_c, vec_c), ...]
...
# [RHS]
class CoupledACCHForce(Problem):
    def custom_init(self, params):
        ## Hu: phase field variable of [c, n]
        self.fe_c = self.fes[0]
        self.fe_n = self.fes[1]
    ...
    def get_universal_kernel(self):
        ...
        def universal_kernel(cell_sol_flat, x, cell_shape_grads, cell_JxW, cell_v_grads_JxW):
            ...
            ######################
            ## This is phase field variable weak form of Cahn-Hilliard - c
            ######################
            ##################### (num_nodes_c, vec_c)
            McV = self.params['McV'] # The CH mobility
            ...
            ######################
            ## This is phase field variable weak form of Allen-Cahn - n
            ######################
            ...
            weak_form = [val_c, val_n] # [(num_nodes_c, vec_c), (num_nodes_n, vec_c)]
...
def get_explicit_dynamics(problem_mass, problem_force):
    ...
    def explicit_euler(sol_list):
        ...
        rhs_c, rhs_n = rhs_list

        sol_c, sol_n = sol_list

        sol_c = sol_c + rhs_c
        sol_n = sol_n + rhs_n

        sol_list = [sol_c, sol_n]
...
def simulation():
    ...
    ### Hu: [c, n]
    problem_mass = CoupledACCHMass(mesh=[mesh, mesh], vec=[1, 1], dim=2, ele_type=[ele_type, ele_type],  additional_info=[params])

    problem_force = CoupledACCHForce(mesh=[mesh, mesh], vec=[1, 1], dim=2, ele_type=[ele_type, ele_type],  additional_info=[params])


    points = problem_mass.fe_n.points
    ...
    rad_list = np.array([20.0/100.0, 8.33333333333/100.0, 20.0/100.0, 8.33333333333/100.0])
    ...
    sol_list = [sol_IC_nucl[0].reshape(problem_mass.fe_c.num_total_nodes, problem_mass.fe_c.vec), 
                sol_IC_nucl[1].reshape(problem_mass.fe_n.num_total_nodes, problem_mass.fe_n.vec)]
    ...
    for i in range(nIter + 1):
        ...
        if (i + 1) % 15000 == 0:
            ...
            logger.debug(f"max of c dofs = {np.max(sol_list[0])}, min of c dofs = {np.min(sol_list[0])}")
            logger.debug(f"max of n1 dofs = {np.max(sol_list[1])}, min of n1 dofs = {np.min(sol_list[1])}")
...
if __name__ == '__main__':
    ...
    print("This is explicit solver for coupled Allen-Cahn and Cahn-Hilliard:", end_time - start_time)
